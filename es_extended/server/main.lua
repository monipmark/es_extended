...
function loadESXPlayer(identifier, playerId, isNew)
	...
	-- Inventory
	if not Config.esx_Inventoryhud then
	...
	if result.position and result.position ~= '' then
		...
		userData.coords = { x = -542.4199829101562, y = -208.6199951171875, z = 37.6500015258789, heading = 205.8 }
	end
	...
	if isNew then
		for k, v in pairs(Config.newplayer) do
...
AddEventHandler('esx:updateCoords', function(coords)
	...
	local playerId = source
	local ped = GetPlayerPed(xPlayer.playerId)
	local playerCoords = GetEntityCoords(ped)

	if xPlayer then
		xPlayer.updateCoords({x = playerCoords.x, y = playerCoords.y, z = playerCoords.z, heading = GetEntityHeading(ped)})
	end
end)
	...
	AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
		...
		elseif type == 'item_account' then
			if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
				...
				TriggerEvent('azael_discordlogs:sendToDiscord', 'COINALIEN', sendToDiscord, sourceXPlayer.source, '^1')	
				...
				TriggerEvent('azael_discordlogs:sendToDiscord', 'COINALIEN', sendToDiscord2, targetXPlayer.source, '^2')
	...
	if eventData.secondsRemaining == 60 then
		CreateThread(function()
			...
			for k, v in pairs(ESX.Players) do
				v.kick('รีเซิฟเเล้ว ไอ้พวกมดงาน')
			end
		end)
	end
end)
...
AddEventHandler("playerConnecting",function(name, setKick, def)
	local source = source
	if restart then
		def.done("เซิฟเวอร์กำลัง รีสตาร์ท")
	end
	
	local Identifier =  ESX.GetIdentifiers(source)
	if not Identifier.steam then
		def.done("กรุณาเปิด steam")
	elseif not Identifier.discord then
		def.done("กรุณาเปิด discord")
	end
end)

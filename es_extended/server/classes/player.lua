...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords)
	...
	function self.kick(reason)
		ESX.DiscordLog({
			...
			Target 	= nil,
			Title 	= 'kick',
			Message = 'reason : ' .. tostring(reason)
				..'\nResource : ' .. (tostring(GetInvokingResource()) or '')
	...
	function self.canCarryItem(name, count)
		if ESX.Items[name] then
			...
			local item = self.getInventoryItem(name)
			return (newWeight <= self.maxWeight) and ( item.limit == -1 or ((count + item.count) <= item.limit))
		else
			print('name : ', name)
			print(ESX.Items[name].weight)
			return false
		end
	end
